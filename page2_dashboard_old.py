...
def store_file_data(store_datatable_data):
    ...
    try:#so, dash convert numpy arrays to list when they are parsed in to a dcc.Store
        ...
        for name, spe, path_stelum, path_pulse in zip(names, spes, path_stelums, path_pulses):
            ...
            stelum_keys, pulse_keys = list(data_dict["stelum"].keys()) + custom_stelum_key, list(data_dict["pulse"].keys())
            ...
            for key in stelum_keys:
                if key not in custom_stelum_key:
                    dict_structure[key].append(data_dict["stelum"][key])
                else: #there's more elegant, but it's easy to keep track of everything this way
                    if key == "N^2":
                        dict_structure[key].append(custom_calc.brunt_vaisala_freq(dict_structure))
                    elif key == "L_1^2":
                        dict_structure[key].append(custom_calc.lamb_freq(dict_structure,1))
                    elif key == "L_2^2":
                        dict_structure[key].append(custom_calc.lamb_freq(dict_structure,2))
                    elif key == "L_3^2":
                        dict_structure[key].append(custom_calc.lamb_freq(dict_structure,3))
                    elif key == "L_4^2":
                        dict_structure[key].append(custom_calc.lamb_freq(dict_structure,4))
...
def update_graph(store_displayed, x_range, y_range, x_scale, y_scale, x_reversed, y_reversed, x_label, y_label, linewidth, linestyle, model_label, color, markers, marker_size, marker_style,  marker_color, modes_displayed, modes_colors, store_line_data, store_active_tab, store_graph_options, value_x, value_y, dropdown_model_name):
    ...
    if n_names > 0:
        ...
        common_params = [linewidth, linestyle, model_label, color]
        tab_params = [markers, marker_size, marker_style,  marker_color]
        ...
        for i, (common_param, tab_param) in enumerate(zip(common_params, tab_params)): #only works cause same size on both tab and commmon params, careful
            ...
            if (tab_param is not None) and (tab_param != ""):
                store_graph_options[f"{dropdown_model_name}_{active_tab}"][i] = tab_param
            else:
                store_graph_options[f"{dropdown_model_name}_{active_tab}"][i] = ''

            if (common_param is not None) and (common_param != ""): #careful, this overrides markers
                store_graph_options[f"{dropdown_model_name}_common"][i] = common_param
            else:
        ...
        if store_graph_options[f"{dropdown_model_name}_common"][0] != '': #linesize
            store_graph_options[f"{dropdown_model_name}_common"][0] = float(store_graph_options[f"{dropdown_model_name}_common"][0])

        if store_graph_options[f"{dropdown_model_name}_{active_tab}"][1] != '': #markersize
...
def formatting_graph_options(sub_store_graph_options, x_values, y_values, value_x, value_y, tab, modes_displayed):
    ...
    if tab == "stelum": #needed because not the same structure for pulse and stelum values
        ...
        if (x_range == "") or (y_range == ""): #avoid double iterations if both are ""
            ...
            x_value = np.array(x_values[0])
            y_value = np.array(y_values[0])

            if x_scale == "log":
                x_value = x_value[x_value > 0]
            if y_scale == "log":
                y_value = y_value[y_value > 0]
            #for N^2, notably (in fact the only example I think)
    ...
    if tab == "pulse": #needed because not the same structure for pulse and stelum values
        ...
        if modes_displayed != None:
            modes_displayed = np.array(list(map(int,modes_displayed.split(",")))) - 1
        else:
            modes_displayed = [0]

        if (x_range == "") or (y_range == ""): #avoid double iterations if both are ""

            min_x, max_x = np.min(x_values[0][modes_displayed[0]]), np.max(x_values[0][modes_displayed[0]])
            min_y, max_y = np.min(y_values[0][modes_displayed[0]]), np.max(y_values[0][modes_displayed[0]])
            ...
            for i,(x_value, y_value) in enumerate(zip(x_values[0],y_values[0])):

                if i in modes_displayed:
...
def updated_graph_pulse(store_graph_options, names, x_values, y_values, value_x, value_y, store_line_data):
    ...
    common_params = [2,"solid",'name','']
    self_params = ["void",8,"circle",'']
    ...
    if n_names > 1:
        ...
        for i, name in enumerate(names):
            ...
            #it's time to set all the defaults yiho
            common_params[-1], common_params[2] = colors[i], name #this is terrible, we're in spagetthi code territory
            self_params[-1] = colors[i]
    ...
    if n_names == 1: #this time, we iterate on modes
        ...
        for i, mode in enumerate(modes):
            ...
            #it's time to set all the defaults yiho
            common_params[-1], common_params[2] = colors[i], name
            self_params[-1] = colors[i]
            ...
            self_params[-1], common_params[-1] = colors[i], colors[i]
                            ...
                            children = [
                                ...
                                html.Div(
                                    ...
                                    children=[
                                        ...
                                        dbc.Input(type="marker-color", id="marker-color", value="#ff0000",style={"height":"15%","width":"80%"}, debounce=True),
                                    ]
                                ),
                                html.Div(
                                    id="color-picker-container",
